...
class calculator:
    def __init__(self):
        ...
        entry = Entry(window, textvariable=self.string, bd=30,width=28)
        entry.grid(row=0, column=0, columnspan=6)
        entry.configure(background="grey",font=('arial',20,'roman'),foreground="red")
        entry.focus()
        ...
        for txt in values:
            ...
            if (txt == '='):
                ...
                btn.grid(row=row, column=col, padx=1, pady=1)
                btn.configure(background="blue",font=('arial',10,'bold'))
